...
public final class Prompts {
    ...
    public static final String CLASSIFY = """
            ...
            取值规则：
            - record：提供或陈述新信息（课程、作业、考试、待办、日程、老师通知、实习/招聘信息等）。
              用户**粘贴岗位 JD 原文**（含公司名、岗位名、岗位职责、任职要求、薪资、工作地点、截止日期等任意组合）属于 record，要入库；
              只有用户**明确要求**分析或制定计划时，才归 match_analysis / study_plan。
            - question：提问、查询（如“今天有什么课”“还有哪些作业没写”）
            - correction：纠正刚记录的信息（如“不对，是11-21”“改一下，地点是A301”）
            - delete：删除某条记录（如“删除作业第3条”“把待办第5条删掉”）
            - match_analysis：用户明确要求分析某条实习与我的匹配度（如"分析匹配度""这个岗位适合我吗"）
            - study_plan：用户明确要求根据岗位 JD 制定学习计划（如"根据这个JD制定学习计划"）
